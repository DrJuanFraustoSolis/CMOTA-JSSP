...
int verifyDominance(double Tk, int counterTks, int counterf0, int limitTksLocal){
	...
	if(counterTks<limitTksLocal){
		T = Tk;
	}
		
	if ( (counterTks==limitTksLocal)&&(change==1) ){
		change=0;
		T=Tk;
	}
		
	//If k is smaller --- more restrictive
	// If k is bigger ------ it gives more diversity						
	if (change!=0){
		k=1;
		//printf("\t k1");
	}else{
	...
	if (  ((makespanNew<makespanOld)&&(tardinessNew<tardinessOld)&&(flowtimeNew<flowtimeOld))
		...
		//BORRAR ESTA CONDICION???				
		if (isSaved(makespanNew, tardinessNew, flowtimeNew, counterf0)==0){  //si Snew no esta grabada se agrega
			counterf0 = saveNewSolution(counterf0);
		}				
	...
	if (  ((makespanOld<makespanNew)&&(tardinessOld<tardinessNew)&&(flowtimeOld<flowtimeNew))
		...
		*/
		...
		if (increase < T) {	//Threshold
			...
			if (isSaved(makespanOld, tardinessOld, flowtimeOld, counterf0)==0){  //IF Snew is already saved it is dont added 
				counterf0 = saveOldSolution(counterf0);
			}
...
int ThresholdAccepting(double T0,double Tf,double alpha,double beta,double Lk){
	...
	int percentage, counterTks=1;
	//printf("\nn: %d", (int) ene);
	percentage = (int) (0.05 * ene);
	//printf("\n5 perc. of n: %d",  percentage);
	int limitTks= (int) ene - percentage;
	//printf("\nLimit of Tks: %d", limitTks);
		...
		for (i = 0; i < intLk; i++) {
			...
			if ( (makespanOld!=makespanNew)&&(tardinessOld!=tardinessNew)&&(flowtimeOld!=flowtimeNew) ){
				if (isSaved(makespanNew,tardinessNew,flowtimeNew,count_f0_run)==0) {
					...
					count_f0_run = verifyDominance(Tk,counterTks,count_f0_run, limitTks);
...
int main(int argc, char *argv[]) {
	...
	for (x=0; x<EXECUTIONS; x++){
		...
		//2. iterate BETWEEN SOLUTIONS FROM THE INITIAL POPULATION AND APPLY THRESHOLD ACCEPTING TO EACH SOLUTION 
		for (k = 0; k < N_INITIAL_SOLUTIONS; k++) {		//iterate between solutions
			...
			count_f0_run = ThresholdAccepting( tT0, tTf, talpha, tbeta, lmin); 	//Output : makespanSA and tardinesSA, flowTimeSA and sOld
